...
class Processor:
    ...
    def load_model(self):
        ...
        self.model = Model(**self.arg.model_args, fch=self.arg.fch, ml_version=self.arg.ml_version,
                           pred_threshold=self.arg.pred_threshold, use_p_map=self.arg.use_p_map, fp_loss=self.arg.fp_loss)
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            # forward
            if self.arg.fch is not None:
                output, metric_loss = self.model(calc_diff_modality(data, **self.train_modality), label, get_metric_loss=True)
            else:
            ...
            if self.arg.fch is not None:
                self.train_writer.add_scalar('metric_loss', metric_loss.mean().data.item(), self.global_step)
                if epoch > self.arg.start_ml_epoch:
                    full_loss += self.arg.w_metric_loss * metric_loss.mean()
