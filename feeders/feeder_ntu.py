...
class Feeder(Dataset):
    ...
    def load_data(self):
        ...
        # print("npz_data", npz_data)
        if self.split == 'train':
            """keys = npz_data.keys()
            for key in keys:
                print("key",key)"""
            self.data = npz_data['x_train']
            # print("self.data.x_train",self.data.shape)
            # print("npz_data['y_train']",npz_data['y_train'])
            self.label = np.where(npz_data['y_train'] > 0)[1]
            # print("self.label",np.where(npz_data['y_train'] > 0))
            self.sample_name = ['train_' + str(i) for i in range(len(self.data))]
            # print("self.sample",self.sample_name.shape)
    ...
    def get_mean_map(self):
        """这段代码的主要功能是通过计算数据在不同维度上的均值和标准差，生成均值图和标准差图。"""
        data = self.data
    ...
    def top_k(self, score, top_k):
        """这段代码的目的是评估给定得分列表中前k个最高得分中有多少个与标签匹配，以衡量模型的准确性。返回的匹配率可以用于评估模型在分类任务中的性能。"""
        rank = score.argsort()
...
def import_class(name):
    """这段代码的目的是实现动态导入类的功能。通过将类名字符串按照点号分割成多个组件，
    然后逐级导入对应的模块或属性，最终返回导入的类。这种方式可以在运行时根据需要动态导入不同的类，提高代码的灵活性和可扩展性。"""
